...
ssh "$PEER" "exec '$BIN' -N -d '$IB_DEV' -i 1 -g '$GID_INDEX' -p '$PORT' -m '$MTU' -n '$ITERS'" &
PID=$!
trap 'kill "$PID" 2>/dev/null || true; wait "$PID" 2>/dev/null || true' EXIT
sleep 1
"$DEXT_DIR/build/mlx_cq_event_gate" -h "${SPARK_CONTROL_HOST:-192.168.100.2}" -p "$PORT" -m "$MTU" \
    -l "${MAC_ROCE_IP:-192.168.200.1}" -a "${MAC_ROCE_MAC:-98:03:9b:80:6a:94}" -r "$REMOTE_MAC" -n "$ITERS"
wait "$PID"
trap - EXIT
