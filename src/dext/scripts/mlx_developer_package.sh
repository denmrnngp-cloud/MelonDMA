...
rm -rf "$DIST_DIR"
mkdir -p "$DIST_DIR/flat/com.melondma.rdma.dext.dext"
make dext shim verbs-compat
cp -R build/MlxRDMA.dext/Contents/. "$DIST_DIR/flat/com.melondma.rdma.dext.dext/"
cp MlxRDMA.dext/Contents/MlxRDMA.entitlements \
  "$DIST_DIR/flat/com.melondma.rdma.dext.dext/"
cp build/librdma_shim.dylib build/libibverbs.dylib "$DIST_DIR/flat/"
plutil -lint "$DIST_DIR/flat/com.melondma.rdma.dext.dext/Info.plist"
plutil -lint "$DIST_DIR/flat/com.melondma.rdma.dext.dext/MlxRDMA.entitlements"
plutil -extract IOKitPersonalities.MlxPCIDriver.MlxBuildTag raw \
  "$DIST_DIR/flat/com.melondma.rdma.dext.dext/Info.plist" > "$DIST_DIR/BUILD_TAG"
plutil -extract CFBundleVersion raw \
  "$DIST_DIR/flat/com.melondma.rdma.dext.dext/Info.plist" > "$DIST_DIR/DEXT_VERSION"
